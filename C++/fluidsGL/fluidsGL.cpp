...
void display(void)
{
	...
	*/
	...
	cudaMemcpy(gvfield, dvfield, sizeof(cData) * DS,
	...
	for(int i=0;i<DIM;i+=stride){
		for(int j=0;j<DIM;j+=stride){
			...
			dx=gvfield[i*DIM+j].x*1000;
			dy=gvfield[i*DIM+j].y*1000;
			tmpx=(float)(j+dx)/DIM;
			tmpy=(float)(i+dy)/DIM;
			glVertex2f(tmpx,tmpy);
			glEnd();
...
int main(int argc, char **argv)
{
	...
	gvfield = (cData *)malloc(sizeof(cData) * DS);
    memset(hvfield, 0, sizeof(cData) * DS);
	memset(gvfield, 0, sizeof(cData) * DS);
